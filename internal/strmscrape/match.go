...
func (s *Service) writeMatchedOpts(ctx context.Context, client *tmdb.Client, g workGroup, info tmdbInfo, overwrite, withTVExtras bool) (epTMDB int, err error) {
	...
	if nfoWriteNeeded(overwrite, nfo) {
		if mediaType == MediaTypeTV {
			if err := writeTVShowNFO(nfo, info.Title, info.TMDBID, info.Plot, info.Year, info.Rating, actors...); err != nil {
				return 0, err
			}
		} else if err := writeMovieNFO(nfo, info.Title, info.TMDBID, info.Plot, info.Year, info.Rating, actors...); err != nil {
