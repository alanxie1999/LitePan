...
// 115 Cookie 关键字段，刷新后的增量 Cookie 也只吸收这几个名字。
var cookieNames = map[string]struct{}{
	"UID":  {},
	"CID":  {},
	"SEID": {},
	"KID":  {},
...
// RefreshAuth 健康检查；115 Cookie 无 refresh_token，仅验 Cookie 是否仍有效。
...
// absorbSetCookie 吸收 115 下发的 UID/CID/SEID/KID 增量 Cookie，合并后回写 account_auth_states。
func (d *Driver) absorbSetCookie(ctx context.Context, header http.Header) {
	...
	for _, c := range parsed {
		if _, ok := cookieNames[c.Name]; ok && c.Value != "" {
